...
@vertex
fn vs_main(in:VertexInput, @builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VertexOutput {
    var out:VertexOutput;
    if(vi ==0 ){
        out.position = vec4f(in.position.x, in.position.y - 0.1, 0.0, 1.0);
    } else if(vi == 1){
        out.position = vec4f(in.position.x+0.5, in.position.y, 0.0, 1.0);
    } else{
        out.position = vec4f(in.position, 0.0, 1.0);
    }
    out.color = in.color;
