...
import { MaterialPhongDescSnippet } from '../../../shaderGraph/snippet/MaterialPhongSnippet';
...
const nanoEntry = async () => {
    ...
    const debugSnippet = new DebugSnippet(compiler);
    const vertexSnippet = new VertexSnippet(compiler);
    const fragmentSnippet = new FragmentDescSnippet(compiler);
    const viewProjectionSnippet = new ViewProjectionSnippet(compiler);
    const meshDescSnippet = new MeshDescSnippet(compiler);
