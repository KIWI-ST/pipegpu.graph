...
fn fs_main(input: ${this.fragmentSnippet.getStructName()})->@location(0) vec4<f32>
{
    ...
    // specular
    let camera_position: vec3<f32> = vec3<f32>(
        ${this.viewSnippet.getVariableName()}.camera_position_x, 
        ${this.viewSnippet.getVariableName()}.camera_position_y,
        ${this.viewSnippet.getVariableName()}.camera_position_z
    );
    let view_dir = normalize(camera_position - input.position_ws.xyz);
